...
図\ref{fig:StatusAndCalibration}にGPIB Interfaceタブの「Status」と「Calibration」グループを示す．
「Calibration」グループ内の「Beta Angle」と「pulsesPerImage」について説明する．
物理的距離検出で計算される「Pulses per pixel ratio」は
平行移動軸に対する割合であり，回転パルス数に換算するためには表\ref{table:PulseStage}に示す定数を利用．
「Beta Angle」は画像一枚当たりに回転するべき角度（単位は度）．それをパルス数に換算した値が「pulsesPerImage」である．
...
館鼻鏡の拡大率を調節しても，自動撮像機能で取得する画像データが連続的になる．

またパルス対ピクセル割合が計算でもとまるため，レコードを一周した後に1500ピクセルだけ，
レコードの中心に向かって平行移動することができる．それ以降撮像する画像データは前の週のデータと
比べてちょうど1500ピクセルだけずれている状況を利用し，音検出アルゴリズム「SigExBot」は
例えば，Round1からRound2のデータに移行するときに，現在追跡中の音溝を新しい画像データの中で用意に
特定することができる．
...
\begin{enumerate}
    ...
    \item 「Camera」タブを開く
    \item カメラ設定が図\ref{cameraSetupFinished}と同じになるように設定
