...
針を用いた再生法では記録媒体が痛んで，再生する度に音質の劣化が伴うことから，
非接触法再生がこの貴重な文化財を傷を付けずにデジタル化できる技術として注目を浴びるようになった．
非接触法の利点はレコードに損害を与えずに再生できるばかりでなく，
割れやひびが入ったレコードも再生可能であることもこの技術のメリットである．

本研究では，SPレコード表面を顕微鏡付きカメラで撮像することにより，全レコードの音溝データを収集する．
一枚一枚の画像の幅が僅か32ピクセルであることがこの研究で用いる「部分読み出し法」の特徴である．
本論文で説明するように，幅が非常に小さい画像データを取得することにより，
画像処理段階で生じる幾何学的な問題を避けることができる，結果としては
画像処理をより高速で行うことができる．

幅が非常に小さい画像データでレコード全面における音溝データを記録しなければならないから，
本研究で巨大なデータ量を管理し，一枚のレコードの完全デジタル化を自動的に行うソフトウェア「SPRecAnalyzer」を開発した．
また，SPRecAnalyzerが生成したデータを対象とする一連の画像処理アルゴリズムを提案した．
画像処理アルゴリズムを実行した後にPCで再生できる，レコードの音声データを再現するWAVファイルが得られる．

\chapter{概要}

\section{実験装置}

SPレコードの撮像に用いた実験装置の模式図を図\ref{fig:experimentalSetup}に示す．図中のDで示すレコードは，2つのパルスステージの上に設置されている．
このうち，平行移動用パルスステージはレコードをカメラの位置に対して水平面内で平行に
（図\ref{fig:experimentalSetup}で言うと左右に）動かす．
この平行移動をパルスモータFによることにより，回転運動と組み合わせることにより，
SPレコード上面の任意の領域を固定されている顕微鏡の下
に持って来ることができる．
...
その仕様はIEEEが固めたことから「IEEE-488」とも知られている．一方，GigEは「Gigabit Ethernet」の略であり，
...
カメラの最大解像度は2456(h)x2058(v)であるが，Variable Partial Scan機能を利用することにより，
...
\subsection{パルスステージ}
本研究で用いた回転ステージはシグマ光機株式会社のSGSP-60YAW-W-0B．
平行移動ステージは同メーカーのSGSP26-200(Z)．
上記の2つを制御するコントローラにはSHOT-102を用いた．
...
パルスモータは位置制御に非常に向いており，本研究でもその性質を活用している．
研究の初期段階で，回転ステージ上に置かれたレコードが滑ることが判明した．
この滑りが原因で，正確な位置制御が困難であった．
滑りの発生を阻止するために，回転ステージの直接上に置かれたアクリル板と回転軸の間に
円形のゴムマットを置いた．同じく，アクリル板とレコードの間にもゴムマットを設置した．

\section{音溝形状と光照射}
顕微鏡を通して撮像された光学像からSPレコードの音溝に記録されたデータを復元するのが本研究目標であるから，
撮像されるデータに音溝の形状が何らかの方法で読み取られなければならない．これを実現するため，
光をレコード面に対して斜めに照射して，音溝の壁の一部が明るく写るようにするのが有力な方法であることが過去の研究でわかった．
...
図\ref{fig:ReflectionOnSPRec}に示すように，SP盤に対して垂直方向に反射する光は，音溝の壁部分が最も多い．
一方，音溝以外の平面部分で反射する光は垂直方向には反射せず，顕微鏡にはほとんど入らない．
この光照射によりカメラと顕微鏡を向いて撮像した画像を図\ref{fig:SprecMicroscopeImgLabeled}に示す．

\begin{figure}[h]
    \begin{center}
        \includegraphics[width=0.5\textwidth, clip]{SprecMicroscopeImgLabeled.eps}
        \caption{反射光をカメルで撮像して得られた画像でーた}
        \label{fig:SprecMicroscopeImgLabeled}
    \end{center}
\end{figure}

本研究の初期段階で照射装置に繋がっている照射ヘッドについて実験を行った．
円形の照射ヘッドで得られた撮像データには，音溝が強く振動している部分においては，
画像処理段階で処理が困難だと考えられる黒い影が出た．そのため，円形照射ヘッドを
短形照射ヘッドへと変更した．
...
そのため，短形の画像を撮影するカメラでその下を回転する円盤の表面を撮像する際どのような問題が生じるかについて，解析的に考察する．
...
本研究では，撮像範囲の幅（図\ref{fig:OverlapProblem_3}中$\overline{AB} = \overline{CD}$）を小さくすることで重複部分の影響を無視できる程度に抑えることで，この問題に対処する．
...
が得られる．本研究で作成したC\#プログラム（詳細な説明は大第4章を参照）の中のキャリブレーション機能を利用した後のパルスステージの場合，変数$\overline{BE}$，$\overline{ME}$と$\overline{MF}$は
...
となる．ここで，前述したように，隣接する画像の隣接重複距離が$2\epsilon$であることを思い出すと最悪の場合の隣接重複距離が
...
図\ref{fig:CurveProblem}において，長方形ABCDがカメラの撮像範囲，TがSPレコードの音溝，MがSPレコードの中心，$\alpha$が次の画像を取るための回転角度だとする．
...
ここまでの幾何学的問題はレコードの性質（音溝がらせん状に円形のレコードに刻まれている）やカメラの撮像範囲が長方形に対し，レコードがカメラの下で回転することに起因している．
ただし，うねり問題は湾曲問題と重複問題と異なり，不可避の問題ではない．
完璧に作られたレコードをそれにぴったりと合う実験装置であれば，うねり問題は生じないと考えられる．
...
図\ref{fig:Uneri}にレコードの一週分の画像データを示す．
ページ内に収まるように，横幅は圧縮されました．この画像データに見られる音溝の形を確認してみると，ゆっくりと上下振動していることが分かる．
この振動は音情報ではなく，単なる穴が偏心しているために生じる．
本研究においては，うねり問題の影響を信号処理で抑制する．
...
欠点として挙げられるのは画像結合が増えること．幅が非常に小さい（本研究では32ピクセル）画像データを数多く取得し，画像処理を通して連結させる過程において，
隣接する画像データの境界面が重複も欠損もなく実際の音溝を連続的に再現できるかどうかが部分読み出し法の難点である．
...
フレームレートと回転速度の設定さえ正しければ隣接する画像データ同士は重複部分も情報損失も起きず，連続的なデータが得られる．
...
SPRecAnalyzerのUIには重要なタブが３つあり，それらの役割について以下の表にまとめる．
...
このタブでの一番左に「Basics」区切りがあり，そこでGPIBをセットアップできる．
...
\begin{figure}[h]
    \begin{center}
        \includegraphics[width=0.9\textwidth, clip]{GPIBSetupFinished.eps}
        \caption{GPIBをセットアップした後のGPIB Interfaceタブの様子}
        \label{fig:GPIBSetupFinished}
    \end{center}
\end{figure}

「Advanced」区切りではパルスステージに対する命令を複数行記述し「Start」ボタンを押すことで
順次実行できる．「Status」はバス状況についての情報が表示される．
「Calibration」区切りはカリブレーション機能を実行，その結果を確認するために設置された．
なお，カリブレーション機能を実行するためにはGPIBとカメラがセットアップ済みでなければならない．

\newpage

\subsection{Cameraタブ}
Cameraタブではカメラ設定を行うことと画像データを取得することができる．
本研究ではカメラのSoftware Trigger機能を利用し画像データの取得タイミングを一枚一枚
トリガーを送ることで決める．「Gain Control」スライダーではカメラのアナログ利得を設定可能．
「Exposure Control」スライダーではカメラの露光時間を設定できる．
本研究の撮像設定では，レコードが一定速度で回転しながら撮像を行うため，
露光時間の値は小さく設定しないと画像データに回転によるぶれが生じる．
本研究で用いた設定の詳細については付属Aを参照．
...
物理的距離検出機能は本研究において非常に重要な機能である．
なぜなら，カメラに撮像命令を送信した際に，回転パルスステージが何パルス進んだ後に次の画像を取るべきかを判定するためには
回転パルスステージにおいて1パルス当たり横方向に何画素進むかといった情報が必要不可欠だからである．
SPRecAnalyzerでは，この重要な変数を「Pulses per pixel ratio」と呼ぶ．

物理的距離検出アルゴリズム内容を以下に示す．

\begin{enumerate}
    \itemsep-0.3pt 
    \item レコードの外側の音溝がカメラ撮像範囲の半分くらいに入る位置まで進む 
    \item 画像一枚を取る
    \item Matlabで画像処理を行い，レコードの外側から見て一番近い音溝の位置情報を検出
    \item 平行移動パルスステージを400パルスだけレコード中心に向かって移動する
    \item 画像一枚を取る
    \item Matlabで画像処理を行い，レコードの外側から見て一番近い音溝の位置情報を検出
    \item 2回行った画像処理の位置情報を比較し，平行移動後，音溝が何ピクセル移動したかを計算する
\end{enumerate}

400パルスだけ移動した際に，表\ref{table:PulseStage}の定数を利用することで何mmだけ平行移動したかということが用意に計算できる．
この結果を用いり，mm/pixel割合を計算することができる．これがいわゆる1ピクセルの物理的サイズ．
これを一画像当たりの回転パルス数の計算に利用する．

物理的距離検出のための画像処理アルゴリズムを付属Bに示す．画像処理内容は２値化，黒枠削除，ラベリング，音溝検出とまとめることができる．
ここでいう「音溝検出」はあくまでも一定のピクセル数を超える集合を音溝だと判断する処理の処理のことである．

\subsection{$\beta$角算出}

$\beta$角は画像一枚当たりに回転すべき角度（単位は度）であり，それをパルス数に換算した値が「pulsesPerImage」である．
SPレコードの回転速度を一定に保ちながらレコードの外側から内側へと移動することに連れて，レコードの表面速度が低下する．
従った，レコードの内側に近づけば近づくほど，一定幅の画像を撮像する際の「pulsesPerImage」値が大きくなるはず．

\begin{figure}[h]
    \begin{center}
        \includegraphics[width=0.9\textwidth, clip]{BetaAngle.eps}
        \caption{$\beta$角が1画像当たり回転すべき角度である}
        \label{fig:BetaAngle}
    \end{center}
\end{figure}
\newpage
この関係を数学的に突き止めてみよう．図\ref{fig:BetaAngle}において，カメラの撮像範囲を長方形ABCD，
$\overline{MH}$をレコードの中心からカメラ撮像領域中心点までの距離とする．また，$\overline{EF}$を撮像画像幅とする．
MHEが直角三角形より

\begin{equation}
    \label{eq:BetaAngle1}
    \frac{\beta}{2} = \arctan{\left(\frac{\overline{MH}}{\overline{HE}}\right)}
\end{equation}
...
\begin{equation}
    \label{eq:BetaAngle1}
    \beta = 2\arctan{\left(\frac{\overline{MH}}{\overline{HE}}\right)}
\end{equation}

と$\beta$角が求まる．物理的距離検出の物理的ピクセルサイズと表\ref{table:PulseStage}に示す定数を利用し，
一画像当たりの回転パルス数（「pulsesPerImage」）を計算できる．

\begin{figure}[h]
    \begin{center}
        \includegraphics[width=0.5\textwidth, clip]{StatusAndCalibration.eps}
        \caption{「Beta Angle」と「pulsesPerImage」はGPIB Interfaceタブで表示される}
        \label{fig:StatusAndCalibration}
    \end{center}
\end{figure}


\subsection{カリブレーション機能}

カリブレーション機能はGPIB Interfaceタブに配置された「Start Calib」ボタンで実行できる．
自動撮像機能を使う前にこの機能を実行する必要がある．
カリブレーション機能の処理内容を以下に示す．
...
「Automatic Image Acq.」タブは自動撮像機能関連の操作ができる．
物理的距離検出アルゴリズムでカメル直結顕微鏡の拡大率に関わらず
正確に１ピクセルの物理的サイズが計算できるため，
顕微鏡の拡大率を変えても，自動撮像機能で取得する画像データが連続的になる．

またパルス対ピクセル割合が計算で求まるため，レコードを一周した後に正確に1500ピクセルだけ，
レコードの中心へと平行移動することができる．平行移動後に撮像する画像データは前の週のデータと
比べてちょうど1500ピクセルずれているため，音検出アルゴリズム「SigExBot」は
...
週別にフォルダに格納される．このデータは次章の画像処理アルゴリズムの対象となる．
